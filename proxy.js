"use strict";
const debug = require("debug")("glorious::proxy");
...
async function runProxy({
  ...
  async function requestHandler(request, response, retries = 0) {
    const currentProxyAddr = rotateProxyAddress();
    const log = msg =>
      debug(`${currentProxyAddr.host} ${currentProxyAddr.port} :: ${msg}`);
    log(`${request.method} ${request.url}`);
    const options = getOptions(request, currentProxyAddr);
  ...
  async function socketHandler(request, socketRequest, retries = 0) {
    ...
    const log = msg =>
      debug(`${currentProxyAddr.host} ${currentProxyAddr.port} :: ${msg}`);
    log(`${request.method} ${request.url}`);
    const options = getOptions(request, currentProxyAddr, true);
  ...
  return new Promise((res, rej) =>
    server.listen(proxyPort, err => (err ? rej(err) : res()))
  );
}
